...
class ImageUpload extends React.Component {
    ...
    handleUpload = (e) => {
        ...
        if (e.currentTarget?.name) {
            const _oldFile = this.state.oldFile && this.state.id ? this.state.oldFile : this.props.preview[e.currentTarget.name];
            this._isMounted && this.setState({ 'oldFile': _oldFile, preview: { ...this.props.preview, [e.currentTarget.name]: URL.createObjectURL(e.currentTarget.files[0]) } });
    ...
    render() {
        ...
        return (<>
            <div
                className={"avatarUpload mt-2 " + (this.props._findKey >= 0 ? "error" : "")}
            >
                <div
                    className={"img " + (this.props.preview[this.props.field.key] ? "" : "show")}
                >
                    <Image
                        src={
                    ...
                    {this.props.preview[this.props.field.key] &&
                        <label
                            ...
                            onClick={e => {
                                e.preventDefault()
                                FileSaver.saveAs(this.props.preview[this.props.field.key], 'dowload');
                            }}
                        >
                            <IconButton
                                color="primary"
