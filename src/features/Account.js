	...
	async (params, {dispatch,rejectWithValue}) => {
		...
		if(_resp?.status=='success'){
			...
			if(typeof _resp.response.data.errors.msg==='string'){
				...
				if(typeof _resp.response.data.errors[0].msg==='string'){
					dispatch(handleFailure(_resp.response.data.errors[0].msg));
					return rejectWithValue({status:'failure'},{msg:_resp.response.data.errors[0].msg});
				}else{
	...
	extraReducers: {
		[login.fulfilled]: (state, action) => {
			if(action?.payload?.status=='success'){
				...
				// chạy sau login slug hiện tiếp theo
				return state;
			}else{
				return state;
			}
