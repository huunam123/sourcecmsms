...
import { setFilter, setFormLayout, handleFailure, handleSuccess, setRoleLayout, setDetailLayout,setImportLayout } from '@features/Status';
...
class ListLayout extends React.Component {
						...
						switch(key){
							...
							case 'tags':
								if (this.state.customSearchStatus == true) {
									...
									if(this.state.values[key]) _filter += '&s='+this.state.values[key].replace(/^0+/, '')+(this.props?.search_fields?'|'+this.props?.search_fields:'');
								}
								break;
							case 'range':
	...
	handleFormLayout=async(e)=>{
		...
		if(_type ==='role'){
			...
			this.props.setFormLayout({status:_status,id:_id,title:_title,width:_width,item_id,module_filter});
		}
	}
	...
	handleClosePopupSendInvitation=async(e)=>{
		await this.setState({isSendInvitation:false,id:'',notification_data:''});
	}
	...
	handleExportLink=async (url,page,limit)=>{
		...
		if(_data?.total > page*limit &&_data.data.length>0){
			...
			if(this.props?.exportFields && this.props.exportFields?.length > 0){
				_dataExport[0].columns= [...this.props.exportFields,...metadata_export].map(field=>({
					label:field.label,
					value: (row)=>{
						let _value = '';
						if (field.format == "dateTime") {
							...
							if(field.key.indexOf('.')>-1){
								...
								if(_n.length == 2 && typeof(row?.[_n[0]])!== 'undefined' && row?.[_n[0]] !== null && Object.keys(row?.[_n[0]]).length>0){
									if(_n[0] == 'metadata'){
										Object.keys(row?.[_n[0]]).forEach((v)=>{
											if(row?.[_n[0]][v]['key'] == _n[1]) _value = row?.[_n[0]][v]['value'] ?? '';
										})
									}else{
										_value= row?.[_n[0]][_n[1]] ? row?.[_n[0]][_n[1]].toString() : '';
									}
								}
	...
	handleSyncStep=async(_id, _o, _l, _type)=>{
		if(this.state.isSync==true){
			if(_id){
				...
				let _result = await postApi(_url,_obj,this.props.stateAccount.access_token);
				if(_result?.status=='success'){
					if((_o + _l)>=_result.total_row){
						...
						this.props.handleSuccess('Send notification:'+(_obj.limit+_obj.offset)+'/'+_result.total_row);
						await this.handleSyncStep(_id,(_o+_l),_l,_type);
					}
				}else{
					await this.setState({isSync:false});
					let _msgError = _result?.response?.data?.errors?.msg ?? (_result?.response?.data?.errors[0]?.msg ?? 'Sync data fail !');
	...
	showValue=(val,column)=>{
		...
		switch(column.type){
			...
			case 'accept':
				...
				break;
			case 'send_status':
				_value=<div className={"accept "+(val[column.key]==true?'text-success':'text-danger')}><i className="fas fa-circle"></i> {val[column.key]==true?'Đã gửi':'Chưa gửi'}</div>;
				break;
			case 'mapping':
		...
		return(
			<React.Fragment>
				<div className={`infoHead ${this.props.stateStatus.asideMinimize ? 'asideMinimize' :''}`}>
					<Stack spacing={2}>
						<h2>{this.props.nameDisplay} <span className="Total">Tổng: {formatNum(this.state.total)}</span></h2>
					</Stack>
						{(this.props.isBtnAdd &&
								...
								<div className={"formSearch "+('me-4')}>
									...
									{this.props.info_provider_select &&
										<div className="infoProviderSelect">
											...
											<Select
												value={this.state.customSelect.info_provider_id ?? 0}
											...
											>
												...
												{this.state.mapping?.info_provider_id&&this.state.mapping.info_provider_id.map(value=>
						   								...
						   								<div className="d-flex">
															...
															}
															{
																...
																<Tooltip title="Chỉnh sửa" placement="top" arrow>
																	<Button className="tt-btn btn btn-light" data-module={this.state.module} data-id={result.id} data-title="Chỉnh sửa" data-width={this.props.maxWidthPopup} onClick={this.handleFormLayout} data-type={(this.props.type?this.props.type:'default')} data-scroll="body" data-item_id={result?.item_id ?? ''} data-module_filter={result?.module ?? ''}>
															...
															}
															{
																...
																<Tooltip title="Gửi email thư mời và QR" placement="top" arrow>
																	<Button className="tt-btn btn btn-light" data-id={result.id}  data-customer-id={result.customer_id} data-activity-tag={result.activity_tag} onClick={this.handleOpenPopupSendInvitation}>
															...
															}
															{
																...
																<Tooltip title="Tải hình thiệp mời" placement="top" arrow>
																	<Button className="tt-btn btn btn-light" data-id={result.id} onClick={this.handleExportInvitation}>
															...
															}
															{
																...
																<Tooltip title="Tải mã QR" placement="top" arrow>
																	<Button className="tt-btn btn btn-light" data-id={result.id} name = "qr_code" onClick={this.handleExportQR}>
															...
															}
															{
																this.props.showActionProcess && <>
																	<Button className="tt-btn btn btn-light" data-module={this.state.module} data-id={result.id} data-title="Process" data-width={this.props.maxWidthPopup} onClick={this.handleFormLayout} data-type={(this.props.type?this.props.type:'default')} data-scroll="body">
						   												<i className="far fa-edit"></i>
						   											</Button>
																</>
															}
						...
						<div className="card-footer">
							<div className="d-flex justify-content-between wp-100">
								...
								<Stack spacing={2} className="pagination">
									<Pagination count={(Math.ceil(total/limit)>Math.ceil(10000/limit))?Math.ceil(10000/limit):Math.ceil(total/limit)} shape="rounded" onChange={this.handlePagination} />
								</Stack>
								}
							</div>
						</div>
				...
				>
					...
					<DialogContent>
						...
						<FormControl>
							...
							>
								<FormControlLabel value="link" control={<Radio />} label="Lấy link file PDF" />
								<FormControlLabel value="email" control={<Radio />} label="Chia sẻ link PDF qua email" />
							</RadioGroup>
							<TextField
								name="shareEmail"
								...
								fullWidth
								variant="standard"
								helperText={(_.findIndex(_errArr, { key: 'email' })) >= 0 ? _errArr[(_.findIndex(_errArr, { key: 'email' }))].msg : ""}
								error={(_.findIndex(_errArr, { key: 'email' })) >= 0 ? true : false}
					...
					<DialogActions>
						<div className="d-flex justify-content-end border-0">
							<Button className="btn btn-light btn-active-primary font-weight-bold" variant="contained" onClick={this.handleClosePopupShare}>
								Hủy
							</Button>
							<Button onClick={this.handleShareApply} className="btn btn-primary font-weight-bold ms-3" variant="contained">
								Chia sẻ
							</Button>
						</div>
					</DialogActions>
							...
							<div className="share-success">
								...
								<div className="share-detail">
									...
									<CopyToClipboard text={this.state.shareLink}>
										...
										<Button className="btn btn-light btn-active-primary font-weight-bold" variant="contained" onClick={this.handleCopy}>
				...
				>
					...
					<DialogActions>
						<div className="d-flex justify-content-end border-0">
							<Button className="btn btn-light btn-active-primary font-weight-bold" variant="contained" onClick={this.handleCloseConfirm}>
								Hủy
							</Button>
							<Button onClick={this.handleStream} data-status='end' className="btn btn-primary font-weight-bold ms-3" variant="contained">
								Đồng ý
							</Button>
						</div>
					</DialogActions>
				...
				>
					...
					<DialogActions>
						<div className="d-flex justify-content-end border-0">
							<Button className="btn btn-light btn-active-primary font-weight-bold" variant="contained" onClick={this.handleClosePopupSendInvitation}>
								Hủy
							</Button>
							<Button onClick={this.handleSendInvitation} data-status='end' className="btn btn-primary font-weight-bold ms-3" variant="contained">
								Đồng ý
							</Button>
						</div>
					</DialogActions>
