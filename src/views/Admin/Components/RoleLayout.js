...
import {FormControl,InputLabel,Checkbox,TextField,Box,DialogContent,MenuItem,FormControlLabel,Tooltip,DialogActions,Button, Tabs, Tab, Select, FormHelperText, Autocomplete} from '@mui/material';
...
class RoleLayout extends React.Component {
	...
	async componentDidMount(){
		...
		if(this.state.id!=''&&!this.props?.router?.query?.token){
			...
			if(_result?.data?.db_filters){
				const filters = JSON.parse(_result.data.db_filters);
				let _fields = {};
				...
				if(_targetModule.length > 0){
					const initData = await Promise.all(_targetModule.map(module=>{
						return fetchApi(process.env.PREFIX_API+`modules/get-target/${module}`,this.props.stateAccount?.access_token).then(resp=>resp ?? {}).catch(e=>{});
					}));
					...
					this._isMounted&&this.setState({ 'data':{...this.state.data,'field_target_field':_dataTargetField,'field_value':_dataTargetValue}});
				}
	...
	handleSubmit=async e=>{
		...
		if(this.state.isLoading==false&&cleanEmpty(this.state.errors).length<=0){
			...
			if(this.state.values?.['fields'] && _.isEmpty(Object.entries(this.state.values?.['fields']))==false){
				let _dataFilters = {};
				for (const [key,value] of Object.entries(this.state.values['fields'])) {
					_dataFilters[key] = {};
					if(value['field_query']&&value['field_field']&&value['field_value']){
						if(value['field_query']=='fq')
							_dataFilters[key][value['field_query']] = `${value['field_field']}:${value['field_value']}`;
						else if(value['field_query']=='fqor')
							_dataFilters[key][value['field_query']] = `${value['field_value']}|${value['field_field']}`;
					}
	...
	handleTabEvent = (e, newValue) => {
		...
		this.setState({ tabEvent: newValue,data:{...this.state.data,'field_module':[..._values]?.map(val=>({value:val,label:val}))}});
	};
	showFields = (field) => {
		let _field;
		let _findKey;
			...
			case "addMoreList":
				...
				_field = <>
					{values.length > 0 && values.map((value, ind) => {
						return <div className="row _r" key={`addMoreList-$${ind}`}>
							<div className="col-lg-6 formControl" key={`addMoreList-type-$${ind}`}>
							...
							<div className="col-lg-6 formControl" style={{ marginBottom: 20 }} key={`addMoreList-key-$${ind}`}>
								<FormControl fullWidth className="selectCustom">
									...
									<TextField
										value={value.key ?? ''}
										helperText={_errArr?.find(err => err.key == `${field.key}[${ind}].key`)?.msg ?? ''}
										error={!!_errArr?.find(err => err.key == `${field.key}[${ind}].key`)?.msg}
										onChange={(e) => {
											const _indexErr = _.findIndex(_errArr, { key: `${field.key}[${ind}].key` });
											if (_indexErr > -1) {
												_errArr = _errArr.filter((_, i) => i != _indexErr);
											}
							...
							<div className="col-lg-4 formControl" style={{ margin: 'auto' }} key={`addMoreList-btn-$${ind}`}>
								<Button
			...
			case "addMore":
				...
				_field = <>
					<div className="row _r">
						<div className="col-lg-2" style={{marginBottom:10}} key={`addMore-field_module-title`}>
							<label
								htmlFor={'field_module'}
								className="formLabel"
							>
								Module
							</label>
						</div>
						<div className="col-lg-2 formControl" style={{marginBottom:10}} key={`addMore-query-title`}>
							<label
								htmlFor={'field_module'}
								className="formLabel"
							>
						...
						<div className="col-lg-2" style={{marginBottom:10}} key={`addMore-field-title`}>
							<label
								htmlFor={'field'}
								className="formLabel"
							>
						...
						<div className="col-lg-2" style={{marginBottom:10}} key={`addMore-target-module-title`}>
							<label
								htmlFor={'field_module'}
								className="formLabel"
							>
						...
						<div className="col-lg-2" style={{marginBottom:10}} key={`addMore-target-field-title`}>
							<label
								htmlFor={'field_module'}
								className="formLabel"
							>
						...
						<div className="col-lg-2" style={{marginBottom:10}} key={`addMore-value-title`}>
							<label
								htmlFor={'field_module'}
								className="formLabel"
							>
						...
						{this.state.data['field_module']?.map((module,ind) => {
							return <>
								<div className="col-lg-2" style={{marginTop:10}} key={`addMore-field_module-${ind}`}>
									{module.value}
								</div>
								<div className="col-lg-2 formControl" key={`addMore-field_query-${ind}`}>
									<FormControl fullWidth className="selectCustom" style={{ marginBottom: 20 }}>
										<Select
											value={values?.[module.value]?.['field_query'] ?? ''}
											onChange={(e) => {
												values[module.value] = values?.[module.value] ?? {};
												values[module.value]['field_query'] = e.target.value;
												this.setState({ 'values':{...this.state.values,[field.targetField]:values} });
											}}
										>
											{[{ value: "fq", label: "Where" },{ value: "fqor", label: "Where not" }].map((v) => (
												<MenuItem key={v.value} value={v.value}>
													{v.label}
												</MenuItem>
											))}
								...
								<div className="col-lg-2 formControl" key={`addMore-field_field-${ind}`}>
									<FormControl fullWidth className="selectCustom" style={{ marginBottom: 20 }}>
										<TextField
											name={'field_field'}
											...
											onChange={(e) => {
												values[module.value] = values?.[module.value] ?? {};
												values[module.value][e.target.name] = e.target.value;
												this.setState({ 'values':{...this.state.values,[field.targetField]:values} });
											}}
										/>
								...
								<div className="col-lg-2 formControl" key={`addMore-field_target_module-${ind}`}>
									<FormControl fullWidth className="selectCustom" style={{ marginBottom: 20 }}>
										<Select
											name={'field_target_module'}
											value={values?.[module.value]?.['field_target_module'] ?? ''}
											onChange={async(e) => {
												values[module.value] = values?.[module.value] ?? {};
												values[module.value][e.target.name] = e.target.value;
												const dataTargetField = this.state.data['field_target_field'] ?? {};
												const dataTargetValue = this.state.data['field_value'] ?? {};
												if(!dataTargetField?.[e.target.value]){
													const value = await fetchApi(process.env.PREFIX_API+`modules/get-target/${e.target.value}`,this.props.stateAccount?.access_token).then(resp=>resp ?? {}).catch(e=>{});
													dataTargetField[e.target.value] = value?.keyTarget ?? [];
													dataTargetValue[e.target.value] = value?.dataTarget ?? [];
												}
												this.setState({ 'data':{...this.state.data,'field_target_field':dataTargetField,'field_value':dataTargetValue},'values':{...this.state.values,[field.targetField]:values} });
											}}
										>
											{this.state['modules']?.map((v) => (
								...
								<div className="col-lg-2 formControl" key={`addMore-field_target_field-${ind}`}>
									<FormControl fullWidth className="selectCustom" style={{ marginBottom: 20 }}>
										<Select
											name={'field_target_field'}
											value={values?.[module.value]?.['field_target_field'] ?? ''}
											onChange={async(e) => {
												values[module.value] = values?.[module.value] ?? {};
												values[module.value][e.target.name] = e.target.value;
												this.setState({'values':{...this.state.values,[field.targetField]:values} });
										...
										>
											{this.state.data['field_target_field']?.[values[module.value]?.['field_target_module']]?.map((v) => (
								...
								<div className="col-lg-2 formControl" key={`addMore-field_value-${ind}`}>
									<FormControl fullWidth className="selectCustom" style={{ marginBottom: 20 }}>
									<Select
											name={'field_value'}
										...
										>
											...
											{values[module.value]?.['field_target_field']=='id' ?this.state.data['field_value']?.[values[module.value]?.['field_target_module']]?.map((v) => (
												<MenuItem key={v.id} value={v.id}>
													{v.name??v.customer_name??v.title}
												</MenuItem>
											)):this.state.data['field_value']?.[values[module.value]?.['field_target_module']]?.map((v) => (
												<MenuItem key={v.id} value={v[values[module.value]['field_target_field']]}>
													{v[values[module.value]['field_target_field']]}
												</MenuItem>
											))}
										</Select>
									</FormControl>
										...
										<div className={"form-root p-0 formLayout-" + module}>
											{
												this.props.tabPlatformData.map((value) => {
													if(value.key == 'info')
														return <div className={`tab-${value.key} ${value.key == tabEvent ? "show" : "hidden"}`} key={value.key}>
															<div className="row">
