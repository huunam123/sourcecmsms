...
class FormLayout extends React.Component {
  ...
  async componentDidMount() {
    ...
    if (this.state.id != "" && !this.props?.router?.query?.token) {
      ...
      if (typeof _result === "object") {
        ...
        // SET METADATA_EXPORT
        if(_result?.data && _result?.data?.metadata_export) _result.data.metadata_export = JSON.parse(_result.data.metadata_export);

         // SET SOURCE FOR invitation_template_render
        if (_result?.data && _result?.data?.invitation_template_render) {
          this.handleSource('template',500,(allFields[_findIndex]?.cdn ?? process.env.CDN_URL_S3)+_result.data.invitation_template_render, false)
        }
    ...
    if (tabName !== null) this._isMounted &&this.setState({ tabPlatForm: tabName });
    this._isMounted &&this.setState({ readOnly: readOnlyField, preview: _preview });
  }
  ...
  async handleGetData(_result,allFields){
    ...
    if (this.props.getData && !this.props?.router?.query?.token) {
      for await (const [key, value] of Object.entries(this.props.getData)) {
        let _result = [];
        if(isArray(value)){
          value.map(i=>{i['name'] = i['name']??i['label'];i['id'] = i['id']??i['value'];return i;});
        ...
        if (_result?.status == "success") {
          ...
          let _defaultValue = (this.props?.fields[_findIndex]?.values && this.props?.fields[_findIndex]?.values.length > 0)
            ...
            : null;
            _data[key] = (typeof(_data[key]) !== 'undefined' && _data[key] != null)
            ? _data[key]
            : (_defaultValue != null
              ? _defaultValue
    ...
    this._isMounted &&  this.setState(
      ...
      async () =>{
        await Promise.all(Object.entries(_data).map(async (v) => {
         let _key = v[0];
          let _findIndex = _.findIndex(allFields, { key: _key});
          if(_findIndex !== -1 && allFields[_findIndex]?.onForChange){
            let _id = _data[_key] ?? [_datas[_key][0]?.id ?? null];
            await this.handleOnForChange(_findIndex,allFields[_findIndex]?.onForChange,_id,_data);
          }
        }))
      ...
      if(_result && _result.length > 0 && _defaultData !== null){
          ...
          async () =>{
            ...
            if(this.props?.fields[_findIndex]?.onForChange){
              this.handleOnForChange(_findIndex,this.props?.fields[_findIndex]?.onForChange,_defaultData,_data)
            }
            await this.handleHiddenFields(_data);
          });
      }
  ...
  async handleHiddenFields(_data){
    ...
    if (_data) {
      ...
      Object.entries(_data).forEach(function(v) {
        ...
        if(_index !== -1){
          Object.entries(hiddenFieldConfigs[_index].values).length>0&&hiddenFieldConfigs[_index].values.forEach(async(item)=>{
            if(item.dataMore) item.values = _dataMore[item.dataMore];
            let _isSuccess = true;
            if(hiddenFieldConfigs[_index].and && Object.entries(hiddenFieldConfigs[_index].and).length>0){
              hiddenFieldConfigs[_index].and.forEach(async(condition)=>{
                Object.entries(condition.values).length>0&&condition.values.forEach(async(i)=>{
                  if(i.keyTagertId == item.keyTagertId){
                    _isSuccess = false;
                    if(i.dataMore) i.values = _dataMore[i.dataMore];
                    if(Object.entries(i.values).length>0&&i.values.includes(_data[condition.key])){
                      _isSuccess = true;
                    }else{
                      _isSuccess = false;
                    }
            ...
            if(_isSuccess&&Object.entries(item.values).length>0&&item.values.includes(v[1])){
              Object.entries(hiddenFieldConfigs[_index].keyTagert[item.keyTagertId]).length>0&&hiddenFieldConfigs[_index].keyTagert[item.keyTagertId].forEach(keyTagert=>{
                hiddenField = hiddenField.filter(field=>field!=keyTagert);
              })
              hiddenFieldConfigs[_index]?.keyTagertHidden&&Object.entries(hiddenFieldConfigs[_index].keyTagertHidden[item.keyTagertId]).length>0&&hiddenFieldConfigs[_index].keyTagertHidden[item.keyTagertId].forEach(keyTagert=>{
                !hiddenField.includes(keyTagert)&&hiddenField.push(keyTagert);
    ...
    if (this.props.customFieldConfigs && !this.props?.router?.query?.token) {
      for (const [key] of Object.entries(this.props.customFieldConfigs)) {
        if(key == _key){
          ...
          let _morekeys = _data?.[_findKey]?.fields ? JSON.parse(_data?.[_findKey]?.fields) :[];
          _morekeys.length>0&&_morekeys.forEach(v =>{
            v.tabKey = 'content'
          })
          morekeys = _morekeys;
  ...
  async handleGetCustomfieldDatas(_key,_value){
    if (this.props.customFieldConfigs && !this.props?.router?.query?.token) {
      for (const [key, value] of Object.entries(this.props.customFieldConfigs)) {
        if(key == _key && value==''){
          ...
          if(find){
            ...
            this._isMounted && this.setState({...this.state,morekeys:_morekeys})
          }
        }
      }
    }
  ...
  componentDidUpdate(prevProps, prevState){
    if(this.state.values["fields"] && this.state.values["fields"]?.length > 0 && this.state.values["fields"] != prevState.values["fields"]){
  ...
  handleUpload=(e)=>{
		...
		if((e.currentTarget.files[0].size/1000)>=2000&&e.currentTarget.files[0].type.indexOf('image')>=0){
			...
			_err.push({key:e.currentTarget?.name,msg:'Video phải nhỏ hơn < 10Mb'});
			this.setState({errors:_err})
			return;
		}
  ...
  handleDeleteImage = (e) => {
    ...
    if (e.currentTarget?.id !== "") {
      ...
      let _fieldKey =  _.findIndex(image_value, {id:parseInt(e.currentTarget.id)})
      image_value[_fieldKey] = {id:parseInt(e.currentTarget.id),delete:true}
  ...
  handleChangeValue = async (e) => {
    ...
    if (_value != "") {
      ...
      if(this.props.fields[_fieldKey]?.tabKey && _findKey > -1){
        _numberErrTab[this.props.fields[_fieldKey].tabKey] = this.state.numberErrTab?.[this.props.fields[_fieldKey].tabKey] -1 ?? 0;
      }
    ...
    if(this.props.fields[_fieldKey]?.key == 'round_count' && _value !=''){
      if (this.state.values['round_range']?.length > _value) {
        this.setState({
          values: { ...this.state.values, 'round_range': this.state.values['round_range'].slice(0,_value) },
        })
      } else if(this.state.values['round_range']?.length<_value){
        const _Arr = [...Array(parseInt(_value-this.state.values['round_range'].length)).keys()]?.map(() => ({}));
        this.setState({
          values: { ...this.state.values, 'round_range': [...this.state.values['round_range'],..._Arr] },
        })
      }
    }
    ...
    this.setState({ values },
      async()=>{
        if (this.props.fields[_fieldKey]?.onForChange) {
          await this.handleOnForChangeOnChange(_fieldKey,this.props.fields[_fieldKey]?.onForChange,_value);
        }
  ...
  async handleOnForChangeOnChange(_fieldKey,_onForChange,_value){
    ...
    for await (const key of Object.keys(_keys)) {
      ...
      _result && _result.length > 0 && this._isMounted &&
      this.setState(
        { data: {...this.state.data, [_key]: _result},
        ...
        async() => {
          let _findIndex = _.findIndex(this.props.fields, { key: _key});
          if(this.props?.fields[_findIndex]?.onForChange){
            await this.handleOnForChangeOnChange(_findIndex,this.props?.fields[_findIndex]?.onForChange,_result[0].id)
          }
          await this.handleHiddenFields(this.state.values);
        }
      );
  ...
  async handleHiddenFieldsOnchange(_data){
    ...
    if(hiddenFieldConfigs){
      ...
      if(_index !== -1){
        Object.entries(hiddenFieldConfigs[_index].values).length>0&&hiddenFieldConfigs[_index].values.forEach(item=>{
          if(item.dataMore) item.values = _dataMore[item.dataMore];
          let _isSuccess = true;
          if(hiddenFieldConfigs[_index].and && Object.entries(hiddenFieldConfigs[_index].and).length>0){
            hiddenFieldConfigs[_index].and.forEach(async(condition)=>{
              Object.entries(condition.values).length>0&&condition.values.forEach(async(i)=>{
                if(i.keyTagertId == item.keyTagertId){
                  _isSuccess = false;
                  if(i.dataMore) i.values = _dataMore[i.dataMore];
                  if(Object.entries(i.values).length>0&&i.values.includes(this.state.values[condition.key])){
                    _isSuccess = true;
                  }else{
                    _isSuccess = false;
                  }
          ...
          if(_isSuccess&&Object.entries(item.values).length>0&&item.values.includes(_value)){
            Object.entries(hiddenFieldConfigs[_index].keyTagert[item.keyTagertId]).length>0&&hiddenFieldConfigs[_index].keyTagert[item.keyTagertId].forEach(keyTagert=>{
              hiddenField = hiddenField.filter(field=>field!=keyTagert);
            })
          }else{
  ...
  handleSubmit = async (e) => {
    ...
    if (this.state.isLoading == false) {
      ...
      allFields.forEach((v)=>{
        if(v.type == "status") _obj[v.key] = this.state.values[v.key] == true || _obj[v.key] == "on" ? true : false;
      })
      ...
      if (_result == "" || _result?.status == "success") {
        ...
        if (typeof _result.response.data.errors?.msg === "string") {
          ...
          let _numErrTab = {};
          if(_result.response.data.errors?.length > 0 && this.props.tabPlatform){
            _result.response.data.errors.forEach(item=>{
              ...
              if(indKey > -1 && this.props.fields[indKey]?.tabKey){
                _numErrTab[this.props.fields[indKey].tabKey] = (_numErrTab?.[this.props.fields[indKey].tabKey] ?? 0) + 1;
              }
            });
          }
          this._isMounted &&
            this.setState({ errors: _result.response.data.errors,numberErrTab:_numErrTab });
        }
      ...
      try {
        ...
        if(this.state.fabric == undefined) { 
          ...
          canvasfabic.forEachObject(function(object) {
            ...
            itemlist[object.id] = {...position,fullWidth:object.width,fullHeight:object.height,center_point:center_point}
          });
          for (const [key, value] of Object.entries(itemlist)) {
            if (key !== 'template'){
              metadata[key] = {
                left: (value.left - itemlist.template.left)/itemlist.template.width*itemlist.template.fullWidth,
                top: (value.top - itemlist.template.top)/itemlist.template.height*itemlist.template.fullHeight,
                width: (value.width/itemlist.template.width)*itemlist.template.fullWidth,
                height: (value.height/itemlist.template.height)*itemlist.template.fullHeight,
                center_point:{
                  x: (value.center_point.x - itemlist.template.left)/itemlist.template.width*itemlist.template.fullWidth,
                  y: (value.center_point.y - itemlist.template.top)/itemlist.template.height*itemlist.template.fullHeight
                }
              }
            }
          }
  ...
  showFields = (field) => {
    ...
    let _readOnly = this.props?.detail ?? this.state.readOnly?.includes(field.key);
    if (_errArr?.length > 0)
    ...
    let _options = [];
    let fieldAddmore = [{ value: "input", label: "Văn bản" }, { value: "image", label: "Hình ảnh" }, { value: "file", label: "File" }]
    ...
    switch (field.type) {
      ...
      case "select":
        _field = (
          <FormControl fullWidth className="selectCustom">
            <Select
              value={
                this.state?.values[field.key] ?? field.defaultValue ??
              ...
              disabled={_readOnly}
            >
              {(field.key.indexOf("metadata") != -1 && this.state.values.fieldData&& this.state.values.fieldData?.length > 0) ? 
                ...
                field.values &&
                  field.values.map((v) => (
                    <MenuItem key={v.value} value={v.value}>
                      {field.showIcon && <i class={v.value} style={{marginRight: 4}}></i>} {" " + v.label}
                    </MenuItem>
                ))
              }
              ...
              {this.state.preview[field.key] && this.state.preview[field.key].map((value,key) => (
                ...
                >
                  <Image
                    src={
                      value?.image_link
                        ? ((field?.cdn ??  process.env.CDN_URL_S3) + value.image_link)
                        : value?.link
                        ? value.link
                        : "/images/transparent.png"
                    }
      ...
      case "textarea":
        ...
        let _par = field.key.split('.');
				if(_par.length==2) _value = (_value[_par[0]]&&_value[_par[0]][_par[1]])?_value[_par[0]][_par[1]]:'';
        ...
        _field = (
          <div className='player-wrapper'>
            <ReactPlayer
              ...
              url={(field?.cdn??process.env.CDN_URL_S3)+this.state.values[field.key]}
              width='100%'
              height='100%'
              controls='true'
            />
      ...
      case "round_range":
        ...
        break;
      case 'video':
        _field = <div className={"avatarUpload mt-2 " + (_findKey >= 0 ? 'error' : '')}>
          <div className={"img " + ((this.state.preview[field.key]) ? '' : 'show')}>
            <video src={(this.state.preview[field.key]) ? this.state.preview[field.key] : '/images/transparent.png'} className="img-cover" />

            <label htmlFor={field.key} className="btnInputFile" onChange={this.handleUpload}>
              <IconButton color="primary" aria-label="upload picture" component="span">
                <i className="fal fa-edit"></i>
              </IconButton>
            </label>
          </div>
          <input accept="video/*" className="inputFile" name={field.key} id={field.key} type="file" onChange={this.handleUpload} />
          <p className="MuiFormHelperText-root">{_findKey >= 0 ? _errArr[_findKey].msg : ''}</p>
        </div>;
        break;
      case "sponsors":
        ...
        _field = (
          <>
            ...
            const anotherValues = values.filter((value,index)=>index != ind);
            const anotherSponsor = anotherValues?.reduce((prev,curr)=>{
              return [...prev,...curr.sponsor_items]
            },[])
            _findKey = _.findIndex(_errArr, { key: `sponsors[${ind}].sponsor_items` });
            return <div className="row" key="">
              <div className="col-lg-2 _r">
                <label
                  htmlFor={'sponsor_type'}
                  className="formLabel"
                >
                  Loại nhà tài trợ
                ...
                <Select
                  ...
                  onChange={(e)=>{
                    ...
                    const is_exclusive = this.state.data['sponsor_types'].find(sponsor_type=>sponsor_type.id == _value)?.is_exclusive ?? false;
                    if(find){
                      ...
                      if(is_exclusive){
                        ...
                        find.sponsor_type_id=_value;
                        find.sponsor_items = []
                        this.setState({values:{...this.state.values,sponsors:[find]}});
                        return;
                      }
                    }
                    this.setState({values:{...this.state.values,sponsors:this.state.values.sponsors}});
                  }}
                  // name={field.key}
                  // error={_findKey >= 0 ? true : false}
                  disabled={_readOnly}
                ...
                >
                  ...
                  {this.state.data['sponsor_types'] && this.state.data['sponsor_types']?.length > 0 &&
                    this.state.data['sponsor_types']?.filter(sponsor_type=>!anotherValues.find(another=>another.sponsor_type_id==sponsor_type.id)).map((value) => (
                      <MenuItem key={value.id} value={value.id}>
                        {field?.mapField
                          ? value[field?.mapField]
                          : value.name
                ...
                <FormControl fullWidth className="selectCustom">
                  <Select
                    ...
                    onChange={(e)=>{
                      e.preventDefault();
                      const _value = e.target.value;
                      const quantity = this.state.values.sponsors?.[ind]?.quantity ?? 10;
                      if(_value && _value?.length > quantity){
                        return
                      }
                      const find = this.state.values.sponsors?.[ind];
                      if(find)
                        find.sponsor_items = _value;
                      this.setState({values:{...this.state.values,sponsors:this.state.values.sponsors}});
                    }}
                    // name={field.key}
                    error={_findKey >= 0 ? true : false}
                    ...
                    renderValue={selected=>{
                      if(selected.length > 0){
                        const _name =selected.map(id=>{
                          const find = this.state.data['sponsor_items']?.find(item=>item.id==id);
                          if(find?.name)
                            return find.name;
                          return id;
                        });
                    ...
                    className="col-lg-12 formControl"
                  >
                    {this.state.data['sponsor_items'] && this.state.data['sponsor_items']?.length > 0 &&
                      this.state.data['sponsor_items'].filter(sponsor=>!anotherSponsor.includes(sponsor.id)).map((value) => (
                        <MenuItem key={value.id} value={value.id}>
                          {field?.mapField
                            ? value[field?.mapField]
                            : value.name
              ...
              <div onClick={(e)=>{
                ...
                if(_readOnly)
                  return
                const sponsors = this.state.values.sponsors.filter((val,index)=>ind != index);
                if(sponsors.length > 0)
                  this.setState({values:{...this.state.values,sponsors}})
              }} className="col-lg-1 _r formControl" style={{margin:'auto',cursor:'pointer'}}>
                <i className="far fa-trash-alt"></i>
              </div>
              {ind == (arr.length -1) &&<div className="col-lg-1 _r formControl">
                <Button
      ...
      case "canvas":
        _field = (
          <div className='canvas-wrapper' id='canvas-wrapper' style={{background: 'gray', minHeight: '500px',width: '100%',height:'100%'}}>
                <canvas
      ...
      case "addMoreList":
        ...
        _field = <>
        {values.length > 0 && values.map((value,ind)=>{
        return field?.elements?.length > 0 ? <div className="row" key={`addMoreList-$${ind}`}>
          {
            field?.elements.map((element,i)=>{
              const [,key_element] = element.key.split('_')
            return <div className={`col-lg-${i==0?3:2} formControl`} style={{marginBottom:20}} key={`form-addMoreList-$${ind}-${i}`}>
            <FormControl fullWidth  className="selectCustom" key={`addMoreList-label-$${ind}-${i}`}>
            <label
                  className="formLabel"
                >
                {element.label}
                ...
                error={!!_errArr?.find(err=>err.key == `${field.key}[${ind}].${key_element}`)?.msg}
                onChange={(e)=>{
                  const _indexErr = _.findIndex(_errArr,{key:`${field.key}[${ind}].${key_element}`});
                  if(_indexErr > -1){
                    _errArr = _errArr.filter((_,i)=>i!=_indexErr);
                  }
                  value[key_element] = e.target.value;
                  this.setState({...this.state,errors:_errArr,values:{...this.state.values,[field.key]:values}})
                }}
              >
                {((element?.data && element.data.length > 0) ? element.data : fieldAddmore).map((v) => (
                    <MenuItem key={v.value} value={v.value}>
                      {v.label}
                    </MenuItem>
                  ))}
                ...
                <TextField
                value={value?.[key_element] ?? ''}
                helperText={_errArr?.find(err=>err.key == `${field.key}[${ind}].${key_element}`)?.msg ?? ''}
                error={!!_errArr?.find(err=>err.key == `${field.key}[${ind}].${key_element}`)?.msg}
                onChange={(e)=>{
                 const _indexErr = _.findIndex(_errArr,{key:`${field.key}[${ind}].${key_element}`});
                 if(_indexErr > -1){
          ...
          <div className="col-lg-1 formControl" style={{margin:'auto 0'}} key={`addMoreList-btn-$${ind}`}>
           <div
             style={{ width: '100%' }}
             onClick={(e) => {
              if(_errArr.length > 0){
                _errArr = _errArr.filter(err=>err['key'].indexOf(`${field.key}[${ind}]`) == -1);
              }
              const res = values.filter((_,i)=>i!=ind);
               this.setState({...this.state,errors:_errArr,values:{...this.state.values,[field.key]:res}})
             }}
           >
             <i className="far fa-trash-alt"></i>
           </div>
        ...
        <div className="row" key={`addMoreList-$${ind}`}>
          <div className="col-lg-4 formControl" style={{marginBottom:20}}>
            <FormControl fullWidth  className="selectCustom" key={`addMoreList-label-$${ind}`}>
              <label
                  className="formLabel"
                >
                Tên
              ...
              <TextField
                value={value.label ?? ''}
                helperText={_errArr?.find(err=>err.key == `${field.key}[${ind}].label`)?.msg ?? ''}
                error={!!_errArr?.find(err=>err.key == `${field.key}[${ind}].label`)?.msg}
                onChange={(e)=>{
                  const _indexErr = _.findIndex(_errArr,{key:`${field.key}[${ind}].label`});
                  if(_indexErr > -1){
                    _errArr = _errArr.filter((_,i)=>i!=_indexErr);
                  }
          ...
          <div className="col-lg-3 formControl" style={{marginBottom:20}}>
          ...
          <div className="col-lg-2 formControl" key={`addMoreList-type-$${ind}`}>
           <FormControl fullWidth  className="selectCustom" style={{marginBottom:20}}>
             <label
               className="formLabel"
             >
               Loại giá trị
           ...
           <Select
             value={value.type ?? ''}
             error={!!_errArr?.find(err=>err.key == `${field.key}[${ind}].type`)?.msg}
             onChange={(e)=>{
              const _indexErr = _.findIndex(_errArr,{key:`${field.key}[${ind}].type`});
              if(_indexErr > -1){
                _errArr = _errArr.filter((_,i)=>i!=_indexErr);
              }
           ...
           <FormHelperText>{_errArr?.find(err=>err.key == `${field.key}[${ind}].type`)?.msg ?? ''}</FormHelperText>
           {value.type == 'select' && 
                  <Box sx={{ flexGrow: 1 }}>
                    <TextField                    
                    multiline
                    ...
                    style={{width: "200px"}}
                      onKeyDown={(e)=>{
                        if (e.key === "Tab") {
                          this.setState({[value.key]: [...this.state?.[value.key]??[], this.tagRef.current.value],values:{...this.state.values,[field.key]:values}})
                          if (_errArr?.[_findKey]?.errors?.[`${field.targetField}field_label`]) {
                            delete _errArr[_findKey]['errors'][`${field.targetField}field_label`];
                          }
                          if (_errArr?.[_findKey]?.errors?.[`${field.targetField}field_key`]) {
                            delete _errArr[_findKey]['errors'][`${field.targetField}field_key`];
                          }
                          value['option'] = [...this.state?.[value.key]??[], this.tagRef.current.value];
                          this.setState({values:{...this.state.values,[field.key]:values}})
                      ...
                      InputProps={{
                        startAdornment: (
                          <Box sx={{ display: "flex", flexWrap: 'wrap',gap: '4px', width: 570}}>
                            { this.state?.[value.key]?.map((val,key)=>(<Box key={key}
                              sx={{
                                background: "#283240",
                                height: "100%",
                                display: "flex",
         ...
         <div className="col-lg-1 formControl" key={`addMoreList-required-$${ind}`}>
           <FormControl fullWidth  className="selectCustom" style={{marginBottom:20}}>
             <label
               className="formLabel"
             >
           ...
           <Select
             ...
             error={!!_errArr?.find(err=>err.key == `${field.key}[${ind}].required`)?.msg}
             onChange={(e)=>{
              const _indexErr = _.findIndex(_errArr,{key:`${field.key}[${ind}].required`});
              if(_indexErr > -1){
                _errArr = _errArr.filter((_,i)=>i!=_indexErr);
              }
           ...
           >
             {[{value:"0",label:"No"},{value:"1",label:"Yes"}].map((v) => (
                 <MenuItem key={v.value} value={v.value}>
                   {v.label}
                 </MenuItem>
               ))}
           </Select>
           <FormHelperText>{_errArr?.find(err=>err.key == `${field.key}[${ind}].required`)?.msg ?? ''}</FormHelperText>
           </FormControl>
         </div>
         <div className="col-lg-1 formControl">
         <FormControl fullWidth  className="selectCustom" style={{marginBottom:20}} key={`addMoreList-sort-$${ind}`}>
         <label
               className="formLabel"
             >
             Vị trí
           ...
           <TextField
             value={value.sort ?? ''}
             onChange={(e)=>{
              const _indexErr = _.findIndex(_errArr,{key:`${field.key}[${ind}].sort`});
              if(_indexErr > -1){
                _errArr = _errArr.filter((_,i)=>i!=_indexErr);
              }
         ...
         <div className="col-lg-1 formControl" style={{margin:'auto'}} key={`addMoreList-btn-$${ind}`}>
           <div
             style={{ width: '100%' }}
             onClick={(e) => {
              if(_errArr.length > 0){
        ...
        _field = <>
          <div className="row _r">
            ...
            {field.elements.map((element,ind)=>
              <div className={`col-lg-${ind==0 ?3:2} formControl`} style={{ marginBottom: 20 }} key={`addMore-${element.label}`}>
              <FormControl fullWidth className="selectCustom">
                <label
                  htmlFor={'field_label'}
                  className="formLabel"
                ...
                <Select
                  ...
                  onChange={(e) => {
                    if (_errArr?.[_findKey]?.errors?.[element.key]) {
                      delete _errArr[_findKey]['errors'][element.key];
                    }
                    this.setState({ [element.key]: e.target.value, errors: _errArr });
                  }}
                  name={element.key}
                  error={!!_errArr?.[_findKey]?.errors?.[element.key]}
                >
                  {((element?.data && element.data.length > 0) ? element.data : fieldAddmore).map((v) => (
                    <MenuItem key={v.value} value={v.value}>
                      {v.label}
                ...
                <TextField
                  ...
                  value={this.state?.[element.key] ?? ''}
                  error={!!_errArr?.[_findKey]?.errors?.[element.key]}
                  helperText={_errArr?.[_findKey]?.errors?.[element.key] ?? ""}
                  onChange={(e) => {
                    if (_errArr?.[_findKey]?.errors?.[element.key]) {
                      delete _errArr[_findKey]['errors'][element.key];
                    }
                    this.setState({ [element.key]: e.target.value })
                  }}
                /></>}
                <FormHelperText>{_errArr?.[_findKey]?.errors?.[element.key] ?? ""}</FormHelperText>
              </FormControl>
            </div>)}
            <div className="col-lg-1 formControl" key={`addMore-btn`}>
              <Button
                ...
                disabled={_errArr?.some(err => err.key == field.key && err?.errors && Object.keys(err.errors).length > 0)}
                onClick={(e) => {
                  const _errors = {};
                  const _obj = {};
                  ...
                  field?.elements.forEach(item => {
                    ...
                    if (!this.state[key]) {
                      _errors[key] = `${item.key.split('_')[1]} là bắt buộc`;
                    } else if (key == 'field_key' && values?.some(value => value?.['key'] == this.state[key])) {
                      _errors[key] = `${item.key.split('_')[1]} đã tồn tại`;
                    }else{
                  ...
                  this.setState({...this.state, values: { ...this.state.values, [field.targetField]: values } })
                }}
              >
            ...
            :<>
            <div className="col-lg-7 formControl" style={{ marginBottom: 20 }} key={`addMore-label`}>
              <FormControl fullWidth className="selectCustom">
                <label
                  htmlFor={'field_label'}
                  className="formLabel"
                ...
                <TextField
                  ...
                  value={this.state?.[`${field.targetField}field_label`]??''}
                  error={!!_errArr?.[_findKey]?.errors?.[`${field.targetField}field_label`]}
                  helperText={_errArr?.[_findKey]?.errors?.[`${field.targetField}field_label`] ?? ""}
                  onChange={(e) => {
                    if (_errArr?.[_findKey]?.errors?.[`${field.targetField}field_label`]) {
                      delete _errArr[_findKey]['errors'][`${field.targetField}field_label`];
                    }
                    ...
                    const field_key = changeToSlug(e.target.value)?.replaceAll('-', '_') + random(1000, 9999);
                    this.setState({ [`${field.targetField}field_label`]: e.target.value, errors: _errArr })
                    this.setState({ [`${field.targetField}field_key`]: field_key, errors: _errArr});
                  }}
                />
                <FormHelperText>{_errArr?.[_findKey]?.errors?.[`${field.targetField}field_label`] ?? ""}</FormHelperText>
              </FormControl>
            </div>
            <div className="col-lg-2 formControl" key={`addMore-type`}>
              <FormControl fullWidth className="selectCustom" style={{ marginBottom: 20 }}>
                ...
                <Select
                  value={this.state?.[`${field.targetField}field_type`]??''}
                  onChange={(e) => {
                    if (_errArr?.[_findKey]?.errors?.[`${field.targetField}field_type`]) {
                      delete _errArr[_findKey]['errors'][`${field.targetField}field_type`];
                    }
                    this.setState({ [`${field.targetField}field_type`]: e.target.value, errors: _errArr });
                  }}
                  name={`${field.targetField}field_type`}
                  error={!!_errArr?.[_findKey]?.errors?.[`${field.targetField}field_type`]}
                >
                  {fieldAddmore.map((v) => (
                    <MenuItem key={v.value} value={v.value}>
                      {v.label}
                ...
                <FormHelperText>{_errArr?.[_findKey]?.errors?.[`${field.targetField}field_type`] ?? ""}</FormHelperText>
              </FormControl>
            </div>
            <div className="col-lg-1 formControl" key={`addMore-required`}>
              <FormControl fullWidth className="selectCustom" style={{ marginBottom: 20 }}>
                <label
                ...
                <Select
                  value={this.state?.[`${field.targetField}field_required`]??''}
                  onChange={(e) => {
                    if (_errArr?.[_findKey]?.errors?.[`${field.targetField}field_required`]) {
                      delete _errArr[_findKey]['errors'][`${field.targetField}field_required`];
                    }
                    this.setState({ [`${field.targetField}field_required`]: e.target.value, errors: _errArr });
                  }}
                  name={`${field.targetField}field_required`}
                  error={!!_errArr?.[_findKey]?.errors?.[`${field.targetField}field_required`]}
                >
                  {[{ value: "0", label: "No" }, { value: "0", label: "Yes" }].map((v) => (
                    <MenuItem key={v.value} value={v.value}>
                      {v.label}
                    </MenuItem>
                  ))}
                </Select>
                <FormHelperText>{_errArr?.[_findKey]?.errors?.['field_required'] ?? ""}</FormHelperText>
              </FormControl>
            </div>
            <div className="col-lg-1 formControl" key={`addMore-sort`}>
              <FormControl fullWidth className="selectCustom" style={{ marginBottom: 20 }}>
                ...
                <TextField
                  ...
                  value={this.state?.[`${field.targetField}field_sort`]??''}
                  onChange={(e) => {
                    if (_errArr?.[_findKey]?.errors?.[`${field.targetField}field_sort`]) {
                      delete _errArr[_findKey]['errors'][`${field.targetField}field_sort`];
                    }
                    this.setState({ [`${field.targetField}field_sort`]: e.target.value })
                  }}
                  type="number"
                  inputProps={{
                    inputMode: "numeric",
                ...
                />
                <FormHelperText>{_errArr?.[_findKey]?.errors?.[`${field.targetField}field_sort`] ?? ""}</FormHelperText>
              </FormControl>
            </div>
            <div className="col-lg-1 formControl" key={`addMore-btn`}>
              <Button
                ...
                onClick={(e) => {
                  const _errors = {};
                  ['field_Key', 'field_Label', 'field_Type'].forEach(item => {
                    const key = field.targetField+item.toLowerCase();
                    if (!this.state[key]) {
                      _errors[key] = `${item.split('_')[1]} là bắt buộc`;
                    } else if (key == 'field_key' && values?.some(value => value?.['key'] == this.state[key])) {
                  ...
                  values.push({
                    ...
                    label: this.state?.[`${field.targetField}field_label`],
                    type: this.state?.[`${field.targetField}field_type`],
                    option: this.state?.[`${field.targetField}field_option`],
                    required: this.state?.[`${field.targetField}field_required`],
                  ...
                  delete this.state?.[`${field.targetField}field_required`];
                  delete this.state?.[`${field.targetField}field_sort`];

                  this.setState({ values: { ...this.state.values, [field.targetField]: values } })
                }}
              >
                <i className="fa fa-plus margin-right-none"></i>
              </Button>
      ...
      case "color":
        ...
        _options = new Set(['linear-gradient(180deg, #0066FF 0%, #00FFD1 100%)', 'linear-gradient(180deg, #0032FF 24.66%, #28EFE3 125%)', 'linear-gradient(180deg, #F40202 24.66%, #200205 125%)', 'linear-gradient(90deg, #D66802 0%, #A97B23 32.85%, #FEE677 65.6%, #E18A2D 100%)', 'linear-gradient(180deg, #F4B840 0.33%, #972121 79.66%)', 'linear-gradient(180deg, #FF0E90 0.33%, #6F0437 79.66%)', 'linear-gradient(261.21deg, #0EFF17 4.91%, #FF0000 32.96%, #0047FF 93.2%)', ...(this.state.data[field.key.split('.')[0]] ?? [])] ?? []);
        _options = [..._options];
        _field = (
          <div className="row _r">
            <div className="col-lg-10">
              <Autocomplete
                ...
                filterOptions={(options, params) => {
                  ...
                  const filtered = _.filter(options, (item) => item.toLowerCase().indexOf(inputValue) > -1);
                ...
                onChange={(e, v) => {
                  ...
                  if (v?.startsWith('Add')) {
                    ...
                    data[field.key.split('.')[0]] = data?.[field.key.split('.')[0]] || [];
                  ...
                  if (field.key.split('.')?.length > 1) {
                    ...
                    values: { ...this.state.values, [field.key]: _value ?? '' },
                    data
                  });
                }
                }
      ...
      case "button_color":
        ...
        break;
      case 'timelinelog':
        _field = <div key={'timelinelog'} style={{ height: '400px' }} className="timeline">
          <PerfectScrollbar>
            {
              this.state.data[field?.key] ?
              (this.state.data[field?.key] ?? []).map(value => {
                try {
                  value.detail = value.detail ? JSON.parse(value.detail) : '';
                } catch (error) {
                  value.detail = '';
                }
                return (
                  <div key={value?.id} className="timelineOnce">
                    <div className="timelineItem">
                      <div className="fw-bolder text-gray-800 timelineLabel">
                        {value?.created_at && moment(value.created_at).format('HH:mm - DD/MM/YYYY')}
                      </div>
                      ...
                      </div>
                      {
                        <div className="fw-mormal text-muted ps-3 timelineContent">
                          <h4 className={typeof value.is_current !== 'undefined' && value.is_current == true ? 'current' : ''}>{value?.email ?? 'Unknown'} - {` ${value?.type ?? 'Unknown'}`}</h4>
        ...
        break;
        case "file":
            _field = (<Link href={(field?.cdn ?? process.env.CDN_URL_S3) + _val[field.key]}>Link</Link>)
          break;
        default:
  ...
  async handleProcessSubmit(e){
    ...
    const success_msg = e?.currentTarget?.dataset?.success_msg ?? 'Cập nhật thành công';
    ...
    if (this.state.isLoading == false && tab_endpoint) {
      ...
      const _result = await putApi(process.env.PREFIX_API + this.state.module + `/${id}/${tab_endpoint}`, _obj, this.props.stateAccount.access_token);
      if (_result == "" || _result?.status == "success") {
   ...
   handleDeleteTag = (value, data) => {    
    ...
    values.map((value,ind)=>{
      ...
      this.setState({[data.key]: newtags, values:{ ...this.state.values ,[data.key]: values}});;
    })
  };
    ...
    return (
      <>
        <form id="form" onSubmit={this.handleSubmit}>
          ...
          {/* ///////////if là có mở { dấu ? là bắt đầu if đó */}
          ...
          {this.props.tabPlatform ? (
            <>
              <DialogContent>
                <Tabs
                  value={tabEvent}
                  onChange={this.handleTabEvent}
                >
                  {
                    this.props.tabPlatformData.map((value, key) => {
                      return (
                        <Tab key={key} label={<Badge style={{padding:5}} invisible={!numberErrTab?.[value.key]} color="error" badgeContent={numberErrTab?.[value.key] ?? 0}>{value.name}</Badge>} value={value.key} />
                      )
                    })
                  }
                </Tabs>
                  ...
                  <div className={"form-root p-0 formLayout-" + module}>
                    {
                      this.props.tabPlatformData.map((value) => {
                        return (
                          <div className={`tab-${value.key} ${value.key == tabEvent ? "show" : "hidden"}`} key={value.key}>
                            {
                              [...this.props.fields,...this.state.morekeys].map((field) => {
                                if(field.tabKey == value.key){
                                  return (
                                    ...
                                    <div key={field.key} className={`_r row ${this.state.hiddenField.includes(field.key)?'hidden':''}`}>
                                      ...
                                      {field.key == 'round_count' && this.state.values?.['round_range']?.map((range, ind, arr) => {
                                        return <div key={ind} className="row" style={{ marginBottom: 10 }}><label
                                          htmlFor={field.key}
                                          className="col-lg-2 formLabel d-flex justify-content-end align-items-center text-lg-end text-start"
                                        >
                                          {`Vòng ${ind + 1}`}
                                          ...
                                          <div className="col-lg-10" style={{ borderBottom: (ind != arr.length - 1) ? '1px solid #eee' : 0, paddingBottom: 10 }}>
                                            {this.showFields({ type: 'round_range', index: ind, minDate: arr?.[ind - 1]?.publish_date })}
                                          </div>
                                        </div>
                                      })}
                                      {(field.key == 'poll_item_type_id' && value.key == 'config_poll') && <div className="_r row">
                                        <div className="col-lg-2"/>
                                        <div noValidate autoComplete="off" className="formSearch col-lg-10">
                                          <div className="block">
                                            <input type="text" value={this.state.search || ''} onChange={(e) => {
                                              e.preventDefault();
                                              this.setState({ ...this.state, search: e.target.value })
                                            }} placeholder="Nội dung tìm kiếm ..." disabled={this.props?.detail || this.state.readOnly.includes('poll_items')}
                                            />
                                            <button onClick={async e => {
                                              e.preventDefault();
                                              const find = this.props.fields.find(val => val.key == 'poll_item_type_id');
                                              if (find) {
                                                const _func = find.onForChange.func;
                                                const data = await _func(this.state.values['poll_item_type_id'], this.props.stateAccount.access_token, this.state.search && `&s=${this.state.search}|name`)
                                                this.setState({ data: { ...this.state.data, [find.onForChange.key]: data } })
                                              }
                                            }}
                                              disabled={this.props?.detail || this.state.readOnly.includes('poll_items')}
                                            >
                                        ...
                                        <div className="col-lg-10">
                                          <List dense sx={{
                                            ...
                                            {this.state.data?.['poll_items']?.map((value) => {
                                              const _readOnly = this.props?.detail || this.state.readOnly.includes('poll_items');
                                              ...
                                              return (
                                                <ListItem
                                                  ...
                                                  secondaryAction={
                                                    <Checkbox
                                                      edge="end"
                                                      checked={this.state?.checked?.some(id => id == value.id) ?? false}
                                                  ...
                                                  <ListItemButton>
                                                    <ListItemAvatar>
                                                      <Avatar
                                                        ...
                                                        src={value.images?.[0]?.image_link ? process.env.CDN_URL_S3 + value.images?.[0]?.image_link : "/images/transparent.png"}
                                                      />
                                                    </ListItemAvatar>
                            ...
                            (field) =>
                              field.col == "left" && (
                                <>
                                  ...
                                  <div className="col-lg-9" style={{borderBottom: (ind != arr.length -1) ? '1px solid grey' : 0 , paddingBottom: 10 }}>
                                    {this.showFields({type:'round_range',index:ind,minDate:arr?.[ind-1]?.publish_date})}
                                  </div>
                                  </div>
                                  })}
                                ...
                                <div className="col-lg-9 formControl">
                                  <div className="d-flex align-items-center">
                                    ...
                                    <TextField
                                      required
                                      onChange={this.handleChangeValue}
                                      value={this.state.values[`revenue[${el}]`]}
                                      name={`revenue[${el}]`}
                                      inputProps={{
                              ...
                              (field) =>
                                ...
                                ((field.key != "influencer_group_id" &&
                                  ...
                                  <div className={`_r ${this.state.hiddenField.includes(field.key)?'hidden':''}`} key={field.key}>
                                    ...
                                    {["status","is_shownumbervote","is_luckyuser"].includes(field.key) ? (
                                      this.showFields(field)
                                    ) : (
